...
```js
闭包是指函数和函数内部能访问的变量之间的关系。在 JavaScript 中，当函数返回后，其内部的变量会继续被保留在内存中，因为其相关的函数中的其他变量可能还需要访问这些变量。这种函数和函数内部变量之间的关系就称为闭包。函数作为参数传递，函数作为返回值返回，函数作为对象的属性存在，这些都是闭包的应用场景。

```

### 34. 比如我有三个接口 他们的返回值作为 第四个个接口的参数 ，如何保证执行顺序呢
